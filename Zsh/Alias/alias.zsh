...
if [[ "$PLATFORM" == "macOS" ]]; then  
  ...
  # Turn ON Metal FPS HUD
  alias metalhud-on='/bin/launchctl setenv MTL_HUD_ENABLED 1 && echo " ✅  MTL_HUD_ENABLED=1 (Metal FPS HUD ON)"'
  # Turn OFF Metal FPS HUD
  alias metalhud-off='/bin/launchctl setenv MTL_HUD_ENABLED 0 && echo " 🚫  MTL_HUD_ENABLED unset (Metal FPS HUD OFF)"'
  alias metalhud-status='echo "Current MTL_HUD_ENABLED=$MTL_HUD_ENABLED"'
  # macOS sama dengan linux
...
#  Python
# ============================
alias py='python3 "$1"' # Mengeksekusi script Python dengan mengeksekusi perintah yang berbeda
# ============================
alias showalias='command showalias'  # memanggil function showalias
alias viewalias='bat ~/.config/zsh/alias.zsh -l sh --style=numbers'
